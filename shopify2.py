...
for collection_url in collection_urls:
    ...
    for page in range(1, total_pages + 1):
        ...
        products = driver.find_elements(By.CSS_SELECTOR, "div.grid__item.grid-product")

        for product in products:
            # Extract image URL
            try:
                image = product.find_element(By.CSS_SELECTOR, "img.grid-product__image")
                image_src = image.get_attribute("src")
                if not image_src:
                    image_src = image.get_attribute("srcset")
                    if image_src:
                        # Take the highest resolution image from srcset
                        image_src = image_src.split(",")[-1].strip().split(" ")[0]
            except Exception as e:
                image_src = None
            ...
            try:
                title = product.find_element(
                    By.CSS_SELECTOR, 
                    "div.grid-product__title.grid-product__title--body"
                ).text.strip()
            except Exception as e:
                title = None
            ...
            try:
                current_price = product.find_element(
                    By.CSS_SELECTOR, 
                    "span.grid-product__price:not(.grid-product__price--original)"
                ).text.strip()
            except Exception as e:
                current_price = None
